...
CREATE FUNCTION commerce.claim_event_outbox (batch_size INTEGER)
RETURNS TABLE (
    id        UUID,
    store_id  UUID,
    event_type TEXT,
...
AS $$
    SELECT
        ...
        event.attempts
    FROM
        integration.claim_routed_event_outbox(
...
AS $$
    SELECT
        ...
        account.store_id
    FROM
        integration.payment_provider_accounts AS account
    WHERE
        account.provider = requested_provider
...
AS $$
    SELECT
        account.id,
        account.webhook_secret_reference
    FROM
        integration.payment_provider_accounts AS account
    WHERE
        account.provider = requested_provider
...
CREATE FUNCTION commerce.claim_webhook_events (batch_size INTEGER)
RETURNS TABLE (
    id        UUID,
    store_id  UUID,
    event_type TEXT,
