CREATE SCHEMA chaos_identity;

CREATE TYPE chaos_identity.user_status AS ENUM ('active', 'disabled');
...
CREATE TABLE chaos_identity.credentials (
    ...
    CONSTRAINT credentials_user_id_fkey              FOREIGN KEY (user_id) REFERENCES chaos_identity.users (id) ON DELETE CASCADE,
...
CREATE TABLE chaos_identity.oauth_clients (
...
CREATE TABLE chaos_identity.oauth_authorization_requests (
    id                          UUID                            NOT NULL,
    client_id                   TEXT                            NOT NULL,
    ...
    CONSTRAINT oauth_authorization_requests_client_fkey         FOREIGN KEY (client_id) REFERENCES chaos_identity.oauth_clients (client_id) ON DELETE CASCADE,
...
CREATE TABLE chaos_identity.oauth_authorization_codes (
    ...
    CONSTRAINT oauth_authorization_codes_client_fkey          FOREIGN KEY (client_id) REFERENCES chaos_identity.oauth_clients (client_id) ON DELETE CASCADE,
    CONSTRAINT oauth_authorization_codes_user_fkey            FOREIGN KEY (user_id) REFERENCES chaos_identity.users (id) ON DELETE CASCADE,
...
CREATE TABLE chaos_identity.oauth_access_tokens (
    ...
    CONSTRAINT oauth_access_tokens_client_fkey          FOREIGN KEY (client_id) REFERENCES chaos_identity.oauth_clients (client_id) ON DELETE CASCADE,
    CONSTRAINT oauth_access_tokens_user_fkey            FOREIGN KEY (user_id) REFERENCES chaos_identity.users (id) ON DELETE CASCADE,
...
CREATE TABLE chaos_identity.oauth_refresh_tokens (
    token_digest                BYTEA                    NOT NULL,
    client_id                   TEXT                     NOT NULL,
    user_id                     UUID                     NOT NULL,
    scope                       TEXT                     NOT NULL,
    ...
    CONSTRAINT oauth_refresh_tokens_client_fkey                   FOREIGN KEY (client_id) REFERENCES chaos_identity.oauth_clients (client_id) ON DELETE CASCADE,
    CONSTRAINT oauth_refresh_tokens_user_fkey                     FOREIGN KEY (user_id) REFERENCES chaos_identity.users (id) ON DELETE CASCADE,
...
CREATE INDEX credentials_user_id_idx ON chaos_identity.credentials (user_id, provider);
CREATE INDEX oauth_authorization_requests_expiry_idx ON chaos_identity.oauth_authorization_requests (expires_at, created_at, id);
CREATE INDEX oauth_authorization_requests_used_cleanup_idx ON chaos_identity.oauth_authorization_requests (used_at, created_at, id) WHERE used_at IS NOT NULL;
CREATE INDEX oauth_authorization_requests_client_idx ON chaos_identity.oauth_authorization_requests (client_id, id);
CREATE INDEX oauth_authorization_codes_expiry_idx ON chaos_identity.oauth_authorization_codes (expires_at, created_at, code_digest);
CREATE INDEX oauth_authorization_codes_consumed_cleanup_idx ON chaos_identity.oauth_authorization_codes (consumed_at, created_at, code_digest) WHERE consumed_at IS NOT NULL;
CREATE INDEX oauth_authorization_codes_client_idx ON chaos_identity.oauth_authorization_codes (client_id, code_digest);
CREATE INDEX oauth_authorization_codes_user_idx ON chaos_identity.oauth_authorization_codes (user_id, code_digest);
CREATE INDEX oauth_access_tokens_user_id_idx ON chaos_identity.oauth_access_tokens (user_id, expires_at);
CREATE INDEX oauth_access_tokens_expiry_idx ON chaos_identity.oauth_access_tokens (expires_at, created_at, token_digest);
CREATE INDEX oauth_access_tokens_revoked_cleanup_idx ON chaos_identity.oauth_access_tokens (revoked_at, created_at, token_digest) WHERE revoked_at IS NOT NULL;
CREATE INDEX oauth_access_tokens_client_idx ON chaos_identity.oauth_access_tokens (client_id, token_digest);
CREATE INDEX oauth_refresh_tokens_user_id_idx ON chaos_identity.oauth_refresh_tokens (user_id, expires_at);
CREATE INDEX oauth_refresh_tokens_expiry_idx ON chaos_identity.oauth_refresh_tokens (expires_at, created_at, token_digest);
CREATE INDEX oauth_refresh_tokens_revoked_cleanup_idx ON chaos_identity.oauth_refresh_tokens (revoked_at, created_at, token_digest) WHERE revoked_at IS NOT NULL;
CREATE INDEX oauth_refresh_tokens_client_idx ON chaos_identity.oauth_refresh_tokens (client_id, token_digest);
CREATE INDEX oauth_refresh_tokens_active_family_idx ON chaos_identity.oauth_refresh_tokens (client_id, user_id, created_at DESC, token_digest) WHERE revoked_at IS NULL;

GRANT USAGE ON SCHEMA chaos_identity TO chaos_identity;

GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA chaos_identity TO chaos_identity;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA chaos_identity TO chaos_identity;

ALTER DEFAULT PRIVILEGES IN SCHEMA chaos_identity GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO chaos_identity;
ALTER DEFAULT PRIVILEGES IN SCHEMA chaos_identity GRANT USAGE, SELECT ON SEQUENCES TO chaos_identity;
