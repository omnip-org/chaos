CREATE TYPE chaos_commerce.cart_status AS ENUM ('active', 'locked', 'completed', 'abandoned');
CREATE TYPE chaos_commerce.order_status AS ENUM ('pending', 'confirmed', 'cancelled');
CREATE TYPE chaos_commerce.order_payment_status AS ENUM ('pending', 'paid', 'failed', 'expired', 'partially_refunded', 'refunded');
CREATE TYPE chaos_commerce.order_refund_status AS ENUM ('pending', 'succeeded', 'failed');
CREATE TYPE chaos_commerce.order_fulfillment_status AS ENUM ('pending', 'shipped', 'delivered', 'cancelled');

CREATE TABLE chaos_commerce.carts (
    id                               UUID                           NOT NULL PRIMARY KEY,
    store_id                         UUID                           NOT NULL,
    channel_id                       UUID                           NOT NULL,
    shopper_id                       UUID                           NOT NULL,
    price_list_id                    UUID                           NOT NULL,
    status                           chaos_commerce.cart_status     NOT NULL DEFAULT 'active',
    ...
    CONSTRAINT carts_store_id_fkey                            FOREIGN KEY (store_id) REFERENCES chaos_commerce.stores (id) ON DELETE CASCADE,
    CONSTRAINT carts_store_id_channel_fkey                    FOREIGN KEY (store_id, channel_id) REFERENCES chaos_commerce.channels (store_id, id),
    CONSTRAINT carts_store_id_shopper_fkey                    FOREIGN KEY (store_id, shopper_id) REFERENCES chaos_commerce.shoppers (store_id, id),
    CONSTRAINT carts_store_id_price_list_fkey                 FOREIGN KEY (store_id, price_list_id) REFERENCES chaos_commerce.price_lists (store_id, id),
...
CREATE TABLE chaos_commerce.cart_lines (
    store_id           UUID        NOT NULL,
    ...
    CONSTRAINT cart_lines_store_id_cart_fkey    FOREIGN KEY (store_id, cart_id) REFERENCES chaos_commerce.carts (store_id, id) ON DELETE CASCADE,
    CONSTRAINT cart_lines_store_id_variant_fkey FOREIGN KEY (store_id, product_variant_id) REFERENCES chaos_commerce.product_variants (store_id, id),
    CONSTRAINT cart_lines_quantity_range_check  CHECK (quantity BETWEEN 1 AND 999)
);

CREATE TABLE chaos_commerce.orders (
    id                              UUID                                    NOT NULL PRIMARY KEY,
    ...
    status                          chaos_commerce.order_status             NOT NULL DEFAULT 'pending',
    payment_status                  chaos_commerce.order_payment_status     NOT NULL DEFAULT 'pending',
    payment_provider_account_id     UUID                                    NOT NULL,
    payment_provider_reference_id   TEXT,
    payment_failure_code            TEXT,
    fulfillment_status              chaos_commerce.order_fulfillment_status NOT NULL DEFAULT 'pending',
    ...
    CONSTRAINT orders_store_cart_context_fkey                 FOREIGN KEY (store_id, cart_id, channel_id, shopper_id) REFERENCES chaos_commerce.carts (store_id, id, channel_id, shopper_id),
    CONSTRAINT orders_store_id_payment_provider_account_fkey  FOREIGN KEY (store_id, payment_provider_account_id) REFERENCES chaos_integration.provider_accounts (store_id, id),
...
CREATE TABLE chaos_commerce.order_lines (
    store_id                UUID        NOT NULL,
    ...
    CONSTRAINT order_lines_store_id_order_fkey           FOREIGN KEY (store_id, order_id) REFERENCES chaos_commerce.orders (store_id, id),
...
CREATE TABLE chaos_commerce.order_refunds (
    id                            UUID                               NOT NULL PRIMARY KEY,
    store_id                      UUID                               NOT NULL,
    order_id                      UUID                               NOT NULL,
    currency                      CHAR(3)                            NOT NULL,
    status                        chaos_commerce.order_refund_status NOT NULL DEFAULT 'pending',
    ...
    CONSTRAINT refunds_store_id_order_currency_fkey            FOREIGN KEY (store_id, order_id, currency) REFERENCES chaos_commerce.orders (store_id, id, currency),
    CONSTRAINT refunds_store_id_payment_provider_account_fkey  FOREIGN KEY (store_id, payment_provider_account_id) REFERENCES chaos_integration.provider_accounts (store_id, id),
...
CREATE TABLE chaos_commerce.order_fulfillments (
    id                     UUID                                      NOT NULL PRIMARY KEY,
    store_id               UUID                                      NOT NULL,
    order_id               UUID                                      NOT NULL,
    status                 chaos_commerce.order_fulfillment_status   NOT NULL DEFAULT 'pending',
    ...
    CONSTRAINT fulfillments_store_id_order_fkey                FOREIGN KEY (store_id, order_id) REFERENCES chaos_commerce.orders (store_id, id),
    CONSTRAINT fulfillments_store_id_provider_account_fkey     FOREIGN KEY (store_id, provider_account_id) REFERENCES chaos_integration.provider_accounts (store_id, id),
...
CREATE INDEX carts_channel_updated_idx ON chaos_commerce.carts (store_id, channel_id, status, updated_at DESC, id DESC);
CREATE UNIQUE INDEX carts_one_active_per_shopper_key ON chaos_commerce.carts (store_id, channel_id, shopper_id) WHERE status = 'active';
CREATE UNIQUE INDEX carts_checkout_idempotency_key_key ON chaos_commerce.carts (store_id, channel_id, shopper_id, checkout_idempotency_key) WHERE checkout_idempotency_key IS NOT NULL;
CREATE INDEX carts_store_shopper_idx ON chaos_commerce.carts (store_id, shopper_id, id);
CREATE INDEX carts_store_price_list_idx ON chaos_commerce.carts (store_id, price_list_id, id);
CREATE INDEX cart_lines_variant_lookup_idx ON chaos_commerce.cart_lines (store_id, product_variant_id, cart_id);
CREATE INDEX orders_channel_created_idx ON chaos_commerce.orders (store_id, channel_id, created_at DESC, id DESC);
CREATE INDEX orders_store_status_id_idx ON chaos_commerce.orders (store_id, status, id DESC);
CREATE INDEX orders_store_contact_email_id_idx ON chaos_commerce.orders (store_id, contact_email, id DESC) WHERE contact_email IS NOT NULL;
CREATE UNIQUE INDEX orders_one_order_per_cart_key ON chaos_commerce.orders (store_id, cart_id);
CREATE INDEX orders_store_shopper_idx ON chaos_commerce.orders (store_id, shopper_id);
CREATE UNIQUE INDEX orders_payment_provider_reference_key ON chaos_commerce.orders (store_id, payment_provider_account_id, payment_provider_reference_id) WHERE payment_provider_reference_id IS NOT NULL;
CREATE UNIQUE INDEX fulfillments_provider_reference_key ON chaos_commerce.order_fulfillments (store_id, provider_account_id, provider_reference_id) WHERE provider_reference_id IS NOT NULL;
CREATE INDEX refunds_order_created_idx ON chaos_commerce.order_refunds (store_id, order_id, created_at DESC);
CREATE INDEX refunds_payment_provider_account_idx ON chaos_commerce.order_refunds (store_id, payment_provider_account_id, order_id);
CREATE UNIQUE INDEX refunds_payment_provider_reference_key ON chaos_commerce.order_refunds (store_id, payment_provider_account_id, payment_provider_reference_id) WHERE payment_provider_reference_id IS NOT NULL;
CREATE INDEX fulfillments_order_created_idx ON chaos_commerce.order_fulfillments (store_id, order_id, created_at DESC);
CREATE INDEX fulfillments_provider_account_idx ON chaos_commerce.order_fulfillments (store_id, provider_account_id, order_id);
CREATE INDEX orders_payment_provider_account_idx ON chaos_commerce.orders (store_id, payment_provider_account_id);

CREATE FUNCTION chaos_commerce.validate_payment_provider_account()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE FUNCTION chaos_commerce.validate_shipping_provider_account()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog
...
CREATE FUNCTION chaos_commerce.prevent_order_identity_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog
...
CREATE TRIGGER orders_payment_provider_capability_check
    ...
    ON chaos_commerce.orders
    FOR EACH ROW EXECUTE FUNCTION chaos_commerce.validate_payment_provider_account();
...
CREATE TRIGGER orders_identity_immutable
    ...
    ON chaos_commerce.orders
    FOR EACH ROW EXECUTE FUNCTION chaos_commerce.prevent_order_identity_change();

CREATE TRIGGER refunds_payment_provider_capability_check
    BEFORE INSERT OR UPDATE OF store_id, payment_provider_account_id
    ON chaos_commerce.order_refunds
    FOR EACH ROW EXECUTE FUNCTION chaos_commerce.validate_payment_provider_account();

CREATE TRIGGER fulfillments_shipping_provider_capability_check
    BEFORE INSERT OR UPDATE OF store_id, provider_account_id
    ON chaos_commerce.order_fulfillments
    FOR EACH ROW EXECUTE FUNCTION chaos_commerce.validate_shipping_provider_account();

ALTER TABLE chaos_commerce.carts ENABLE ROW LEVEL SECURITY;
ALTER TABLE chaos_commerce.cart_lines ENABLE ROW LEVEL SECURITY;
ALTER TABLE chaos_commerce.orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE chaos_commerce.order_lines ENABLE ROW LEVEL SECURITY;
ALTER TABLE chaos_commerce.order_refunds ENABLE ROW LEVEL SECURITY;
ALTER TABLE chaos_commerce.order_fulfillments ENABLE ROW LEVEL SECURITY;

CREATE POLICY store_isolation ON chaos_commerce.carts
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

CREATE POLICY store_isolation ON chaos_commerce.cart_lines
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

CREATE POLICY store_isolation ON chaos_commerce.orders
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

CREATE POLICY store_isolation ON chaos_commerce.order_lines
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

CREATE POLICY store_isolation ON chaos_commerce.order_refunds
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

CREATE POLICY store_isolation ON chaos_commerce.order_fulfillments
    USING (store_id = nullif(current_setting('app.store_id', true), '')::uuid)
    WITH CHECK (store_id = nullif(current_setting('app.store_id', true), '')::uuid);

GRANT SELECT, INSERT, UPDATE, DELETE
    ON chaos_commerce.carts,
       chaos_commerce.cart_lines,
       chaos_commerce.orders,
       chaos_commerce.order_lines,
       chaos_commerce.order_refunds,
       chaos_commerce.order_fulfillments
    TO chaos_runtime;

REVOKE DELETE, TRUNCATE ON chaos_commerce.carts,
    chaos_commerce.orders,
    chaos_commerce.order_refunds,
    chaos_commerce.order_fulfillments
    FROM chaos_runtime;
REVOKE UPDATE ON chaos_commerce.orders FROM chaos_runtime;
GRANT UPDATE (
    payment_status,
    ...
    ON chaos_commerce.orders TO chaos_runtime;
REVOKE UPDATE, DELETE, TRUNCATE ON chaos_commerce.order_lines FROM chaos_runtime;

GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA chaos_commerce TO chaos_runtime;

REVOKE ALL ON FUNCTION chaos_commerce.validate_payment_provider_account() FROM PUBLIC;
REVOKE ALL ON FUNCTION chaos_commerce.validate_shipping_provider_account() FROM PUBLIC;
REVOKE ALL ON FUNCTION chaos_commerce.prevent_order_identity_change() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION chaos_commerce.validate_payment_provider_account() TO chaos_runtime;
GRANT EXECUTE ON FUNCTION chaos_commerce.validate_shipping_provider_account() TO chaos_runtime;
GRANT EXECUTE ON FUNCTION chaos_commerce.prevent_order_identity_change() TO chaos_runtime;
