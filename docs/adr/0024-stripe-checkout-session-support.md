...
can recover the same payment after a browser back, remount, or lost response; it
...
idempotency key are not duplicated in Chaos. A retry of the same Cart checkout
request reuses the same Order and Session; the Order's `pi_`
...
```text
POST /webhooks/v1/payment/stripe/{provider_account_id}
```
...
the API process stopped after the Order commit or a Provider call failed), the
same Cart checkout request may retry Session creation with the same
Order-derived Provider idempotency key. Because the return URL is not persisted,
the retry requires a new validated return URL. A Provider error leaves the
Order pending for another Cart-based retry; a successful Provider response
without a client action is a terminal local failure that cancels the Order and
