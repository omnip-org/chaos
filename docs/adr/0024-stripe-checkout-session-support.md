...
```text
POST /storefront/v1/webhooks/stripe/{store_id}
```

`store_id` is the UUID generated by Chaos. It is the explicit routing hint; the
Webhook signing secret authenticates the request. The verifier resolves the
Store's single Stripe Provider Account, tries its active and still-valid
previous signing secret, and then writes the event with that Provider Account's
