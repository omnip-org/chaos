...
Alert on sustained growth of `chaos_analytics_collection_rate_limited_total`. HTTP 429 responses include a `Retry-After` value of at most 60 seconds; clients should retain stable event IDs and retry after that interval. A broad Store-and-Channel spike indicates integration misuse or abuse, while a single anonymous identity reaching 120 events per minute usually indicates an SDK loop. Redis loss intentionally returns HTTP 503 for collection and must not fall through to PostgreSQL.

Trusted commerce facts use the `analytics.commerce_fact_ingestor` Outbox owner. Alert through the existing per-owner backlog metrics and `integration.event_consumer_backlog()` when any `analytics.*` fact event is older than 60 seconds, retries, or reaches dead letter. The worker reclaims one-minute leases and uses the Outbox event ID as the immutable fact ID. Repair the owning commerce row or fact mapper and replay the original event; never insert a replacement fact with a different ID or edit amounts in `analytics.commerce_facts`.
...
Alert on `chaos_analytics_retention_oldest_expired_seconds` and non-zero `chaos_analytics_retention_expired_behavior_events`, `chaos_analytics_retention_expired_sessions`, or `chaos_analytics_retention_expired_identity_links` lasting more than two maintenance intervals. The worker retries after five seconds on failure and otherwise purges at most 1,000 rows of each type per minute. Confirm worker health and database locks before adding capacity. Never extend expiry or directly delete immutable evidence to clear the alert; repair and replay the bounded retention path. A shorter Store policy intentionally shortens existing expiry and cannot be reversed for already deleted data.

Alert when `chaos_analytics_erasure_oldest_pending_seconds` exceeds 60 seconds or `chaos_analytics_erasure_pending` grows continuously. Each request is one database transaction: raw events, processing rows, sessions, identity links, completion state, and counts commit together or all roll back. Confirm database locks and worker health, then allow the normal worker to retry. Never mark a request completed or delete rows manually. The audit selector is an internal opaque identifier and must not be copied into logs, tickets, or metrics. Analytics erasure never authorizes deletion of Customer, Order, Payment, Refund, Fulfillment, Return, or statutory records.
...
Shipping Provider Accounts use the same outbound rotation discipline but have no inbound secret. Submit a new Provider Key through `POST .../provider-secrets`, update the Store account with its replacement `env://CHAOS_SHIPPING_SECRET_*` or `enc://<base64>` reference, verify the returned rollback deadline without expecting either reference in the response, and exercise a non-destructive rate quote before considering the previous EasyPost key retired. An `enc://` value is decrypted on demand and requires no API restart. Keep the account disabled if the deployment does not contain the named adapter or the default origin is no longer operationally valid.

For shipping tracking incidents, inspect `chaos_shipping_tracking_due`, `chaos_shipping_tracking_processing`, `chaos_shipping_tracking_dead_letter`, and `chaos_shipping_tracking_oldest_due_seconds`. For submitted Label refunds, use the corresponding `chaos_shipping_cancellation_due`, `chaos_shipping_cancellation_processing`, `chaos_shipping_cancellation_dead_letter`, and `chaos_shipping_cancellation_oldest_due_seconds` metrics. A lease older than one minute is recoverable by another instance. Confirm Provider availability and credential resolution before replaying exhausted work; reset a due time and its attempt counter only through an audited operational change. Never mark a Fulfillment delivered to clear backlog; the Provider observation must drive the application transition. A Label refund status does not prove parcel interception and must not be used to cancel a Fulfillment.
