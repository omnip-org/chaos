...
| 9 — Analytics and Attribution | In progress | Transactional commerce events, Store and Channel boundaries, search, immutable snapshots, and queue infrastructure; a versioned first-party browser event contract; consent-gated, size-, time-, Store/Channel-rate-, and anonymous-identity-rate-bounded Storefront batch collection; Store-scoped deduplication; immutable raw behavior evidence; Store/Channel/RLS isolation; a dependency-free Storefront SDK that emits visible-and-focused engagement with bounded retry; recoverable server-side sessionization with a 30-minute inactivity boundary, out-of-order bridge merging, and a four-hour engagement cap; immutable Store Analytics Policy versions with independently controlled behavior collection, advertising exports, identity linking, and 1-400 day retention; bounded automatic retention deletion; authenticated and consent-evidenced anonymous-to-Customer links; idempotent, atomic data-subject erasure with immutable completion counts; transactional, recoverable canonical facts for Order creation, Payment capture, Refund success, Fulfillment shipment, and Return completion; delayed, versioned, rebuildable first- and last-touch Order attribution with explicit consent, policy, input-watermark, direct-traffic, retention, and erasure evidence; and Store/Channel-isolated daily behavior, currency-safe commerce, and versioned attribution read models queried through a dedicated read-only, low-capacity, timeout-bounded PostgreSQL pool | Meta CAPI and GA4 destinations. |
