...
| 9 — Analytics and Attribution | In progress | Transactional commerce events, Store and Channel boundaries, search, immutable snapshots, and queue infrastructure; a versioned first-party browser event contract; consent-gated, size- and time-bounded Storefront batch collection; Store-scoped deduplication; immutable raw behavior evidence with explicit 30-day retention; Store/Channel/RLS isolation; a dependency-free Storefront SDK that emits visible-and-focused engagement with bounded retry; and recoverable server-side sessionization with a 30-minute inactivity boundary, out-of-order bridge merging, and a four-hour engagement cap | Collection rate limiting; configurable consent and retention policy; deletion and identity unlinking; attribution; trusted commerce-fact ingestion; isolated reporting storage; Meta CAPI and GA4 destinations. |
