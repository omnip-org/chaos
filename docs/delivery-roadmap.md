...
| 10 — Extensibility and Ecosystem | In progress | Versioned Admin, Store, and Webhook contracts; scoped API keys; search read model; Store-owned custom Domains bound to Web Sales Channels; one-time digest-backed DNS TXT verification; immutable lifecycle events; RLS administration; and exact-hostname public resolution restricted to active verified context | Collections and media; localization; outbound webhooks; MCP; generated SDKs; compatibility automation; third-party application workflows. |
