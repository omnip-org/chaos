...
| 6 — Transaction Hardening | In progress | Existing idempotency, inbox, outbox, RLS, and state-machine foundations; possession-bound shopper credentials and constrained Cart-to-Payment ownership lineage | Lease recovery; deterministic expiry scheduling; event consumer ownership; graceful worker drain; process-crash and replay evidence. |
