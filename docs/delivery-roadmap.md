...
| 10 — Extensibility and Ecosystem | In progress | Versioned Admin, Store, and Webhook contracts; scoped API keys; search read model; Store-owned custom Domains bound to Web Sales Channels; one-time digest-backed DNS TXT verification; immutable lifecycle events; exact-hostname public resolution; and Store-owned manually ordered Collections with independent lifecycle, Sales Channel publication, audit, RLS, and publication-safe Storefront filtering | Media; localization; outbound webhooks; MCP; generated SDKs; compatibility automation; third-party application workflows. |
