...
| 10 — Extensibility and Ecosystem | In progress | Versioned Admin, Store, and Webhook contracts; scoped API keys; search read model; Store-owned custom Domains bound to Web Sales Channels; one-time digest-backed DNS TXT verification; immutable lifecycle events; exact-hostname public resolution; Store-owned manually ordered Collections with independent lifecycle and publication; and verified direct-upload Product Media with an S3-compatible port, audit, RLS, and ready-only Storefront filtering | Localization; outbound webhooks; MCP; generated SDKs; compatibility automation; third-party application workflows. |
