...
| 8 — Provider Integrations | In progress | Stripe Connect direct-charge decision and production adapter; automated live onboarding readiness and responsibility gating; PaymentIntent and Refund dispatch; possession-bound client handoff; provider administration; external secret-resolution port and environment adapter; 24-hour credential rollback and webhook verification overlap; timestamped signed Stripe webhooks; Fulfillment state; transactional events; and SMTP-based authentication delivery | Add periodic Stripe reconciliation; implement Resend notification delivery; implement a shipping provider port and first adapter. |
