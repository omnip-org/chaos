...
| 8 — Provider Integrations | In progress | Stripe Connect direct-charge decision and production adapter; automated live onboarding readiness, periodic reconciliation, fail-closed expiry, and responsibility gating; PaymentIntent and Refund dispatch; possession-bound client handoff; provider administration; external secret-resolution port and environment adapter; 24-hour credential rollback and webhook verification overlap; timestamped signed Stripe webhooks; Fulfillment state; transactional events; capability-specific email provider port; Resend authentication and semantic Order-confirmation delivery; transactional versioned requests; recoverable leases; stable idempotency; bounded failure classification; signed raw-body webhook reconciliation; Store-isolated hard-bounce, complaint, and provider suppression; SMTP development fallback; capability-specific shipping provider and secret-resolution ports; a real EasyPost HTTP adapter for rating, label purchase, refund requests, and tracking refresh; and Store-owned EasyPost administration with a default origin, write-only credential references, and 24-hour rollback | Persist rate and label evidence; wire quote, purchase, cancellation, and tracking reconciliation into Fulfillment APIs and workers. |
