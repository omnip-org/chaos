...
| 9 — Analytics and Attribution | In progress | Transactional commerce events, Store and Channel boundaries, search, immutable snapshots, and queue infrastructure; a versioned first-party browser event contract; consent-gated, size- and time-bounded Storefront batch collection; Store-scoped deduplication; immutable raw behavior evidence with explicit 30-day retention; Store/Channel/RLS isolation; and a dependency-free Storefront SDK that emits visible-and-focused engagement with bounded retry | Collection rate limiting; configurable consent and retention policy; sessionization; attribution; trusted commerce-fact ingestion; isolated analytics read models; Meta CAPI and GA4 destinations. |
