...
| 4 — Payments | In progress | Provider-neutral application port and sandbox adapter; Store-owned Provider account administration with immutable identity mapping, write-only secret-manager references, enable/disable lifecycle, deterministic checkout resolution, RLS, and Admin contracts; currency-safe Payment Attempt and Refund state machines; authenticated webhook inbox; transactional outbox; recoverable leases; bounded worker drain; retry/dead-letter handling; atomic capture reconciliation | Verify one production payment adapter end to end. |
