...
| 5 — Operations | In progress | Partial Fulfillment and shipment tracking; recoverable event-driven Order fulfillment and delivery reconciliation; authorized Returns with deterministic line-total allocation, receipt disposition, inventory restocking, and exactly-once refund coordination; transactional search events; rebuildable Store-isolated search; optional OTLP trace export; bounded business and queue metrics; capacity harness; operations runbooks; Admin and Store contracts | Retain an executed capacity report and close the security and operational release gates. |
...
| 8 — Provider Integrations | In progress | Stripe Connect direct-charge decision and production adapter; PaymentIntent and Refund dispatch; possession-bound client handoff; provider administration; external secret-resolution port and environment adapter; timestamped signed Stripe webhooks; Fulfillment state; transactional events; and SMTP-based authentication delivery | Enforce automated Stripe onboarding readiness before live enablement; add overlap credential rotation and reconciliation; implement Resend notification delivery; implement a shipping provider port and first adapter. |
