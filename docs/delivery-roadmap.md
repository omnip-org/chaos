...
| 8 — Provider Integrations | In progress | Stripe Connect direct-charge decision and production adapter; automated live onboarding readiness, periodic reconciliation, fail-closed expiry, and responsibility gating; PaymentIntent and Refund dispatch; possession-bound client handoff; provider administration; external secret-resolution port and environment adapter; 24-hour credential rollback and webhook verification overlap; timestamped signed Stripe webhooks; Fulfillment state; transactional events; capability-specific email provider port; Resend authentication delivery with stable idempotency and bounded failure classification; and SMTP development fallback | Implement durable semantic notification delivery, signed Resend webhook ingestion, and suppression handling; implement a shipping provider port and first adapter. |
