...
`POST /admin/v1/merchant-accounts/{merchant_account_id}/stores/{store_id}/analytics-erasure-requests` accepts exactly one anonymous ID or internal Customer ID and returns HTTP 202 with an immutable pending request. `GET .../analytics-erasure-requests/{request_id}` reports completion and deletion counts. The worker atomically deletes matching raw events, processing rows, sessions, and identity links across a bounded batch; Sales Customer and immutable commerce evidence remain intact. Duplicate requests and requests for data already absent safely complete with zero counts.
