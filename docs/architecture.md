...
- Store-owned Shipping Services and destination regions are Fulfillment data. Sales requests a server-authoritative quote and freezes the selected service, amount, currency, and delivery estimate in Checkout and Order snapshots. Store-owned Shipping Provider Accounts hold provider identity, enablement, a default origin, and opaque credential references; secret material remains external. External shipping providers remain Fulfillment infrastructure adapters. Notification providers deliver semantic requests without owning commerce state.
