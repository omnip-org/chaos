...
- `pgmq` does not require a background worker. `0004_integration.sql` creates the shared event, provider-webhook, and capability queues. `0011_topic_routing.sql` adds native topic routing (`pgmq.bind_topic`/`pgmq.send_topic`, available since `pgmq` v1.11.0): `integration.publish_commerce_event` fans one routing key out to every bound queue (`analytics_capi_queue`, `notification_email_queue`, `search_index_queue`), and `integration.claim_topic_queue`/`finish_topic_event` are the one generic claim/finish pair every topic-routed consumer reuses — there is no delivery-row table backing them, only PGMQ's own message lifecycle (`pgmq.archive()` is the dead-letter surface once retries are exhausted). Runtime roles do not receive direct access to the `pgmq` schema; narrowly granted routines in `integration` and `commerce` claim, finish, and publish messages while updating authoritative records in the same transaction.
