...
| 9 — Analytics and Attribution | Partial | The first-party browser event contract, consent-gated Storefront collection path, active-engagement SDK, recoverable server-side sessionization, versioned Store policy, automatic retention deletion, consent-aware identity links, and data-subject erasure are delivered. Rate limiting, attribution, trusted commerce facts, isolated reporting storage, and conversion destinations remain open. |
