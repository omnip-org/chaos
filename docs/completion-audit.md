...
| 9 — Analytics and Attribution | Partial | The first-party browser event contract, consent-gated Storefront collection path, active-engagement SDK, and recoverable server-side sessionization are delivered. Rate limiting, configurable policy, deletion and identity unlinking, attribution, trusted commerce facts, isolated reporting storage, and conversion destinations remain open. |
...
| Contract and runtime evidence | The Store OpenAPI 3.1 contract documents every allowlisted event and response count. The real-router PostgreSQL matrix proves storage, consent discard, replay deduplication, scope denial, event-property rejection, same-ID Store isolation, cross-account RLS, out-of-order session convergence, engagement aggregation, stale-lease recovery, and stale-owner rejection on a clean PostgreSQL 18 bootstrap. |
| Open gates | Collection rate limiting, configurable Store policy, erasure and identity-unlinking jobs, attribution, trusted commerce-fact ingestion, isolated reporting storage, and Meta CAPI/GA4 adapters are not yet delivered. |
