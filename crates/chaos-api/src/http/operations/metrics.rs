...
async fn scrape(State(state): State<ApiState>) -> impl IntoResponse {
    ...
    if let Ok((heartbeat_age, healthy)) = sqlx::query_as::<_, (f64, bool)>(
        "SELECT heartbeat_age_seconds, healthy FROM integration.worker_health()",
    )
    .fetch_one(&pool)
    .await
    {
        ::metrics::gauge!("chaos_worker_heartbeat_age_seconds").set(heartbeat_age);
        ::metrics::gauge!("chaos_worker_healthy").set(f64::from(healthy));
    }
    if let Ok((due, retrying, expiring, action_required)) =
        sqlx::query_as::<_, (i64, i64, i64, i64)>(
            "SELECT * FROM commerce.provider_readiness_metrics()",
        )
        .fetch_one(&pool)
        .await
    {
    ...
    if let Ok((pending, processing, dead_letter, suppressed, oldest)) =
        sqlx::query_as::<_, (i64, i64, i64, i64, f64)>(
            "SELECT * FROM integration.email_delivery_metrics()",
        )
        .fetch_one(&pool)
        .await
    {
    ...
    if let Ok((due, processing, dead_letter, oldest)) = sqlx::query_as::<_, (i64, i64, i64, f64)>(
        "SELECT * FROM commerce.shipping_tracking_metrics()",
    )
    .fetch_one(&pool)
    .await
    {
    ...
    if let Ok((due, processing, dead_letter, oldest)) = sqlx::query_as::<_, (i64, i64, i64, f64)>(
        "SELECT * FROM commerce.shipping_cancellation_metrics()",
    )
    .fetch_one(&pool)
    .await
    {
