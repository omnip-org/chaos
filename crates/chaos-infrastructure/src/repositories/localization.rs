...
impl CatalogLocalizationRepository for PostgresCatalogLocalizationRepository {
    async fn store_locales(
        ...
        let result = load_store_locales(&mut transaction, store_id).await?;
    ...
    async fn enable_locale(
        ...
        let result = load_store_locales(&mut transaction, store_id)
            .await?
    ...
    async fn set_default_locale(
        ...
        sqlx::query("INSERT INTO commerce.store_locales (store_id,locale,created_by_user_id,created_at) VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING")
    ...
    async fn disable_locale(
        ...
        let default = lock_default_locale(&mut transaction, store_id).await?;
    ...
    async fn product_translation(
        ...
        let mut transaction = self.begin(&actor).await?;
        let result =
            load_product_translation(&mut transaction, store_id, product_id, locale).await?;
        transaction.commit().await.map_err(database_error)?;
        Ok(result)
    }
    ...
    async fn upsert_product_translation(
        ...
        require_translatable_locale(&mut transaction, store_id, translation.content.locale())
            .await?;
    ...
    async fn remove_product_translation(
        ...
        require_product(&mut transaction, store_id, product_id).await?;
        sqlx::query("DELETE FROM commerce.product_translations WHERE store_id=$1 AND product_id=$2 AND locale=$3")
    ...
    async fn collection_translation(
        ...
        let mut transaction = self.begin(&actor).await?;
        let result =
            load_collection_translation(&mut transaction, store_id, collection_id, locale).await?;
        transaction.commit().await.map_err(database_error)?;
        Ok(result)
    }
    ...
    async fn remove_collection_translation(
        ...
        require_collection(&mut transaction, store_id, collection_id).await?;
        sqlx::query("DELETE FROM commerce.collection_translations WHERE store_id=$1 AND collection_id=$2 AND locale=$3").bind(store_id.as_uuid()).bind(collection_id.as_uuid()).bind(locale.as_str()).execute(&mut *transaction).await.map_err(database_error)?;
    ...
    async fn remove_media_translation(
        ...
        require_media(&mut transaction, store_id, product_id, media_asset_id).await?;
        sqlx::query("DELETE FROM commerce.media_asset_translations WHERE store_id=$1 AND product_id=$2 AND media_asset_id=$3 AND locale=$4").bind(store_id.as_uuid()).bind(product_id.as_uuid()).bind(media_asset_id.as_uuid()).bind(locale.as_str()).execute(&mut *transaction).await.map_err(database_error)?;
...
async fn require_store(
    ...
    lock_default_locale(tx, store).await.map(|_| ())
}
async fn require_translatable_locale(
    ...
    let default = lock_default_locale(tx, store).await?;
    if default == locale.as_str() {
        return Err(ApplicationError::Conflict {
...
async fn require_product(
    ...
    require_status(tx, store, "products", id.as_uuid(), "product").await
}
async fn require_collection(
    ...
    require_status(tx, store, "collections", id.as_uuid(), "collection").await
}
...
async fn replay_product_translation(
    ...
    load_product_translation(tx, store, product, locale)
        .await?
        .ok_or_else(invalid_snapshot)
}
...
async fn replay_collection_translation(
    ...
    load_collection_translation(tx, store, collection, locale)
        .await?
        .ok_or_else(invalid_snapshot)
}
...
async fn replay_media_translation(
    ...
    load_media_translation(tx, store, product, media, locale)
        .await?
        .ok_or_else(invalid_snapshot)
}
