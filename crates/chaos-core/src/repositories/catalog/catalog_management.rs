...
#[derive(Clone)]
pub struct PostgresCatalogManagementRepository {
    pool: PgPool,
}

impl PostgresCatalogManagementRepository {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

pub(crate) struct PostgresCatalogManagementTransaction {
...
mod tests {
    ...
    async fn manages_product_content_lifecycle_and_publication_atomically() {
        ...
        let service = CatalogManagement::new(Arc::new(PostgresCatalogManagementRepository::new(
            runtime_pool,
        )));
        assert!(matches!(
            service
    ...
    async fn publishable_machine_actor_cannot_administer_catalog_under_rls() {
        ...
        let publishable_machine = AdminActor::Machine(crate::ports::MachineActor {
